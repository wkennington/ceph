...
CompatSet Monitor::get_supported_features()
{
  ...
  ceph_mon_feature_incompat.insert(CEPH_MON_FEATURE_INCOMPAT_OSD_ERASURE_CODE_V2);
...
void Monitor::apply_quorum_to_compatset_features()
{
  ...
  if (quorum_features & CEPH_FEATURE_ERASURE_CODE_PLUGINS_V2) {
    new_features.incompat.insert(CEPH_MON_FEATURE_INCOMPAT_OSD_ERASURE_CODE_V2);
  }
...
void Monitor::apply_compatset_features_to_quorum_requirements()
{
  ...
  if (features.incompat.contains(CEPH_MON_FEATURE_INCOMPAT_OSD_ERASURE_CODE_V2)) {
