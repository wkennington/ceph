    ...
    map<RGWObjCategory, RGWStorageStats>& stats, string *max_marker)
{
  ...
  bool has_shards = (headers.size() > 1);
  for(; iter != headers.end(); ++iter) {
    accumulate_raw_stats(iter->second, stats);
    compose_bucket_version(bucket_ver, has_shards, iter->first, iter->second.ver);
    compose_bucket_version(master_ver, has_shards, iter->first, iter->second.master_ver);
    compose_bucket_marker(max_marker, has_shards, iter->first, iter->second.max_marker);
  }
  return 0;
}
