...
{
  switch (stage) {
  case 0: // kick off the backtrace fetch
  {
    if (is_dirty()) {
  ...
  case 1: // check the backtrace and
  {
    ...
    if (is_dir())
      ...
      pool = inode.layout.fl_pg_pool;
    inode_backtrace_t memory_backtrace;
    build_backtrace(pool, memory_backtrace);
    bool equivalent, divergent;
    int memory_newer = memory_backtrace.compare(c->results->backtrace.ondisk_value,
